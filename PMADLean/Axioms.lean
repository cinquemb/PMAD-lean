...
def IsDynamicallyStable (lambda : ℝ) : Prop :=
  lambda ≤ 0
...
/-- Axiom A2: An attractor set A is structurally stable under a global 
    ...
    where the stability condition (IsDynamicallyStable lambda) forces the dynamic 
    trajectory to restrict to tighter open sheets over time. -/
def AttractorSet 
    (A : Set (PhaseState N)) 
    (lambda : ℝ := -1) 
    : Prop :=
  IsDynamicallyStable lambda → 
    ∀ (ϕ : Trajectory N),
